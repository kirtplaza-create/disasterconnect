<template>
  <div class="fade-up">
    ...
    <!-- Stats -->
    <div class="stat-grid">
      <div class="stat-box"><div class="stat-icon"><CloudLightning :size="20" color="var(--color-danger)" /></div><div class="stat-val" style="color:var(--color-danger);">{{ activeDisasters }}</div><div class="stat-label">ACTIVE DISASTERS</div></div>
      <div class="stat-box"><div class="stat-icon"><Users :size="20" color="var(--color-warn)" /></div><div class="stat-val" style="color:var(--color-warn);">{{ totalFamilies.toLocaleString() }}</div><div class="stat-label">FAMILIES IN NEED</div></div>
      <div class="stat-box"><div class="stat-icon"><Package :size="20" color="var(--color-success)" /></div><div class="stat-val" style="color:var(--color-success);">{{ totalItemsDonated.toLocaleString() }}</div><div class="stat-label">TOTAL ITEMS DONATED</div></div>
      <div class="stat-box"><div class="stat-icon"><CheckCircle :size="20" color="var(--color-success)" /></div><div class="stat-val" style="color:var(--color-success);">{{ confirmed }}</div><div class="stat-label">DONATIONS CONFIRMED</div></div>
    </div>

    <!-- Urgent needs -->
    <div style="margin-bottom:24px;">
      <div class="section-title"><AlertTriangle :size="18" color="var(--color-danger)" /> Urgent Needs Right Now</div>
      <div class="urgent-grid">
        <div v-for="r in urgentReports" :key="r.id" class="urgent-card" :style="{ borderLeft:`3px solid ${sevColor(r.severity)}` }" @click="$emit('navigate','donor-donate')">
          ...
          <div class="urgent-needs">
            <span><Box :size="12" /> Needs: <span style="color:var(--color-warn);font-weight:700;">{{ r.foodNeeded }} packs</span></span>
            <span><Droplet :size="12" /> <span style="color:var(--color-accent);font-weight:700;">{{ r.waterNeeded }}L</span></span>
          </div>
    ...
    <!-- How it works -->
    <div class="card">
      <div class="section-title" style="color:var(--color-success);"><Search :size="18" /> How Your Donation is Tracked</div>
      <div class="how-grid">
...
const howSteps = [
  { step:'1', icon: HeartHandshake, color: 'var(--color-danger)', title:'You Donate',    desc:'Log your donation with items and quantity' },
  { step:'2', icon: CheckCircle,    color: 'var(--color-accent)', title:'LGU Verifies',  desc:'Operations center confirms your donation' },
  { step:'3', icon: Truck,          color: 'var(--color-warn)',   title:'Dispatched',    desc:'Relief goods are packed and sent to barangay' },
  { step:'4', icon: Home,           color: 'var(--color-success)', title:'Received',      desc:'Barangay captain signs for delivery' },
]
</script>
...
.hero{background:linear-gradient(135deg,color-mix(in srgb, var(--color-success), transparent 93%),color-mix(in srgb, var(--color-accent), transparent 96%));border:1px solid color-mix(in srgb, var(--color-success), transparent 80%);border-radius:12px;padding:2.5rem 2rem;margin-bottom:24px;text-align:center;position:relative;overflow:hidden}
.hero-icon{font-size:48px;margin-bottom:12px}
.hero-title{font-size:26px;font-weight:900;margin-bottom:8px}
.hero-sub{font-size:14px;color:var(--text-secondary);max-width:520px;margin:0 auto 20px;line-height:1.8}
...
.urgent-card{background:var(--bg-surface);border:1px solid var(--border-color);border-radius:8px;padding:1.25rem;cursor:pointer;transition:all .2s}
.urgent-card:hover{transform:translateY(-2px);border-color:color-mix(in srgb, var(--color-success), transparent 80%)}
...
.urgent-muni{font-size:11px;color:var(--text-secondary)}
.urgent-desc{font-size:12px;color:var(--text-secondary);margin-top:8px;line-height:1.6}
.urgent-needs{display:flex;gap:10px;flex-wrap:wrap;font-size:11px;color:var(--text-secondary)}
.urgent-cta{font-size:11px;color:var(--color-success);margin-top:8px;font-weight:700}
...
.how-desc{font-size:11px;color:var(--text-secondary);line-height:1.6}

.btn-primary{background:var(--color-success);color:var(--bg-body);border:none;border-radius:6px;padding:10px 24px;font-size:14px;font-weight:700;cursor:pointer;font-family:'Outfit',sans-serif;transition:opacity .2s}
.btn-primary:hover{opacity:.85}
.btn-outline{background:transparent;border:1px solid var(--color-success);color:var(--color-success);border-radius:6px;padding:10px 24px;font-size:14px;font-weight:700;cursor:pointer;font-family:'Outfit',sans-serif;transition:all .2s}
.btn-outline:hover{background:color-mix(in srgb, var(--color-success), transparent 90%)}
