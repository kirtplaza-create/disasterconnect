    ...
    <div class="card-list">
      <div v-for="r in reports" :key="r.id" class="report-card" :style="{ borderLeft:`3px solid ${sevColor(r.severity)}` }">
        <div style="flex:1;">
          ...
          <div class="needs-grid">
            <div v-for="row in needsRows(r)" :key="row.label" class="need-box">
              ...
              <div class="need-val" :style="{ color: row.color }">{{ row.val?.toLocaleString() }} <span style="font-size:10px;color:var(--text-secondary);">{{ row.unit }}</span></div>
            </div>
          </div>
          <div class="people-row">
            <span v-for="[icon,val,label] in [[Users,r.families,'families'],[Users,r.individuals,'individuals'],[Footprints,r.evacuees,'evacuees']]" :key="label" class="people-chip">
              <component :is="icon" :size="12" style="display:inline; margin-right:4px;" /> <span style="color:var(--text-primary);font-weight:700;">{{ val }}</span> {{ label }}
            </span>
          </div>
        </div>
        <div style="flex-shrink:0;display:flex;flex-direction:column;gap:8px;align-items:flex-end;">
          ...
          <span style="font-size:11px;color:var(--text-secondary);">{{ r.muni }}</span>
        </div>
      </div>
    </div>
  </div>
...
const needsRows  = r => [
  { icon: Box, label: 'Food Packs', val: r.foodNeeded, unit: 'packs', color: 'var(--color-warn)' },
  { icon: Droplet, label: 'Water', val: r.waterNeeded, unit: 'liters', color: 'var(--color-accent)' },
  { icon: Stethoscope, label: 'Medical Kits', val: r.medNeeded, unit: 'kits', color: 'var(--color-danger)' }
]
...
.report-card{background:var(--bg-surface);border:1px solid var(--border-color);border-radius:8px;padding:1.25rem;display:flex;align-items:flex-start;gap:16px}
...
.report-date{font-size:11px;color:var(--text-secondary);font-family:'DM Mono',monospace;margin-left:auto}
.report-desc{font-size:12px;color:var(--text-secondary);margin-bottom:12px;line-height:1.7}
.needs-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-bottom:8px}
.need-box{background:var(--bg-body);border:1px solid var(--border-color);border-radius:6px;padding:8px 10px}
.need-label{font-size:11px;color:var(--text-secondary);margin-bottom:2px}
.need-val{font-size:14px;font-weight:700}
.people-row{display:flex;gap:12px;flex-wrap:wrap}
.people-chip{font-size:12px;color:var(--text-secondary)}
...
.btn-primary{background:var(--color-success);color:var(--bg-body);border:none;border-radius:6px;padding:9px 18px;font-size:13px;font-weight:700;cursor:pointer;font-family:'Outfit',sans-serif;transition:opacity .2s}
