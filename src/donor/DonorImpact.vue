<template>
  <div class="fade-up">
    ...
    <div class="stat-grid">
      <div class="stat-box"><div class="stat-icon"><HeartHandshake :size="20" color="var(--color-success)" /></div><div class="stat-val" style="color:var(--color-success);">{{ donations.length }}</div><div class="stat-label">TOTAL DONATIONS</div></div>
      <div class="stat-box"><div class="stat-icon"><Package :size="20" color="var(--color-accent)" /></div><div class="stat-val" style="color:var(--color-accent);">{{ totalItems.toLocaleString() }}</div><div class="stat-label">ITEMS DONATED</div></div>
      <div class="stat-box"><div class="stat-icon"><Users :size="20" color="var(--color-warn)" /></div><div class="stat-val" style="color:var(--color-warn);">{{ totalFamilies }}</div><div class="stat-label">FAMILIES HELPED</div></div>
      <div class="stat-box"><div class="stat-icon"><CheckCircle :size="20" color="var(--color-success)" /></div><div class="stat-val" style="color:var(--color-success);">{{ confirmed }}</div><div class="stat-label">DELIVERIES CONFIRMED</div></div>
    </div>
    ...
    <div class="card">
      ...
      <div class="leaderboard">
        <div v-for="(d,i) in leaderboard" :key="d.id" class="leader-row">
          <div class="rank-circle" :style="{ background: i<3?rankBg[i]:'color-mix(in srgb, var(--border-color), transparent 80%)', border:`1px solid ${i<3?rankColor[i]:'var(--border-color)'}`, color: i<3?rankColor[i]:'var(--text-secondary)' }">{{ i+1 }}</div>
          <div style="flex:1;">
            <div style="font-size:13px;font-weight:700;">{{ d.donor }}</div>
            <div style="font-size:11px;color:var(--text-secondary);">For {{ linkedBrgy(d.reportId) }}</div>
          </div>
          <div style="text-align:right;margin-right:12px;">
            <div style="font-size:14px;font-weight:900;color:var(--color-success);font-family:'DM Mono',monospace;">{{ itemTotal(d).toLocaleString() }}</div>
            <div style="font-size:10px;color:var(--text-secondary);">total items</div>
          </div>
...
const PIE_C       = ['var(--color-accent)','var(--color-success)','var(--color-warn)','var(--color-danger)']
const rankColor   = ['var(--color-warn)','var(--text-secondary)','var(--color-accent)']
const rankBg      = [
  'color-mix(in srgb, var(--color-warn), transparent 87%)',
  'color-mix(in srgb, var(--text-secondary), transparent 90%)',
...
onMounted(async () => {
  ...
  const GRID = { color: getVar('--border-color') }
  const TICK = { color: getVar('--text-secondary'), font:{ size:10, family:'DM Mono, monospace' } }

  if (barChartRef.value) new Chart(barChartRef.value, {
    type:'bar',
    data:{ labels: monthlyData.map(d=>d.month), datasets:[{ label:'Donations', data:monthlyData.map(d=>d.donations), backgroundColor:getVar('--color-success'), borderRadius:3 }] },
  ...
  if (pieChartRef.value) new Chart(pieChartRef.value, {
    type:'doughnut',
    data:{ 
      labels:donorTypes.value.map(d=>d.name), 
      datasets:[{ 
        data:donorTypes.value.map(d=>d.value), 
        backgroundColor:PIE_C.map(c => c.startsWith('var') ? getVar(c.match(/var\((.+)\)/)[1]) : c), 
        borderWidth:0 
      }] 
    },
...
.chart-title{font-size:13px;font-weight:700;margin-bottom:14px;color:var(--color-success)}
.leaderboard{display:flex;flex-direction:column;gap:8px}
.leader-row{display:flex;align-items:center;gap:14px;padding:10px 12px;background:var(--bg-body);border:1px solid var(--border-color);border-radius:6px}
