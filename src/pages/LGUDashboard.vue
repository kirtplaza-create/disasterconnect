<template>
  <div class="fade-up">
    ...
    <div class="stat-grid">
      <div class="stat-box">
        <div class="stat-value" style="color: #00D4FF;">{{ reports.length }}</div>
        <div class="stat-label">Active Disasters</div>
      </div>
      <div class="stat-box">
        <div class="stat-value" style="color: #FF3B5C;">{{ criticalCount }}</div>
        <div class="stat-label">Critical Alerts</div>
      </div>
      <div class="stat-box">
        <div class="stat-value" style="color: #FF6B35;">{{ totalAffected.toLocaleString() }}</div>
        <div class="stat-label">Total Affected</div>
      </div>
      <div class="stat-box">
        <div class="stat-value" style="color: #FFD23F;">{{ totalEvacuees.toLocaleString() }}</div>
          ...
          <tr v-for="r in reports" :key="r.id" class="table-row">
            ...
            <td class="td">
              <button v-if="r.aiAnalysis" class="result-btn"
                :style="{ background: sevColor(r.aiAnalysis.severityLevel) + '12', border: `1px solid ${sevColor(r.aiAnalysis.severityLevel)}55` }"
            ...
            <td class="td">
              <button v-if="r.needsAssessment" class="result-btn"
                style="background:#00E5A00e;border:1px solid #00E5A044;"
                @click="openModal(r, 'needs')">
                <div class="result-btn-title" style="color:#00E5A0;">{{ r.needsAssessment.foodPacks }} packs</div>
            ...
            <td class="td">
              <button v-if="r.allocation" class="result-btn"
                style="background:#FF6B350e;border:1px solid #FF6B3544;"
                @click="openModal(r, 'matching')">
                <div class="result-btn-title" style="color:#FF6B35;">{{ r.allocation.allocations?.[0]?.foodPacks }} packs</div>
            ...
            <td class="td">
              <div style="display:flex;gap:3px;">
                <div v-for="(step, i) in pipelineSteps(r)" :key="i"
                  class="pipeline-dot"
                  :title="step.label"
                  :style="{ background: step.done ? (step.ai ? '#00D4FF' : '#00E5A0') : '#1A2535' }" />
              </div>
    ...
    <div class="two-col">
      ...
      <div class="card">
        ...
        <div v-for="inv in inventory" :key="inv.id" class="inv-row">
          <div class="inv-top">
            <span :style="{ fontSize:'12px', color: inv.low ? '#FF3B5C' : '#E2EAF4' }">
              {{ inv.item }} 
              <AlertTriangle v-if="inv.low" :size="12" style="display:inline; margin-left:4px;" />
            </span>
            <span :style="{ fontSize:'12px', fontFamily:'DM Mono,monospace', color: inv.low ? '#FF3B5C' : '#00D4FF' }">{{ (inv.available || 0).toLocaleString() }} {{ inv.unit }}</span>
          </div>
          <div class="progress-bg">
            <div class="progress-fill" :style="{ width: (inv.low ? 18 : 65) + '%', background: inv.low ? '#FF3B5C' : '#00E5A0' }" />
          </div>
        </div>
      ...
      <div class="modal fade-up">
        ...
        <div class="modal-tabs">
          <button v-for="tab in modalTabs" :key="tab.id" class="modal-tab"
            :style="{
              borderBottomColor: modalTab === tab.id ? '#00D4FF' : 'transparent',
              color: !tab.has ? '#4A6080' : modalTab === tab.id ? '#00D4FF' : '#E2EAF4',
              opacity: tab.has ? 1 : 0.4,
              cursor: tab.has ? 'pointer' : 'not-allowed',
            }"
            @click="tab.has && (modalTab = tab.id)">
        ...
        <div class="modal-body">
          ...
          <template v-if="modalTab === 'analysis'">
            <div v-if="modalReport.aiAnalysis" class="fade-up">
              ...
              <div class="score-grid-large" style="margin-top: 8px;">
                <div class="analysis-score-card" :style="{ borderTop: `4px solid ${sevColor(modalReport.aiAnalysis.severity?.level || modalReport.aiAnalysis.severityLevel)}` }">
                  <div class="score-header">
                    ...
                    <span class="score-level-badge" :style="{ background: sevColor(modalReport.aiAnalysis.severity?.level || modalReport.aiAnalysis.severityLevel) + '22', color: sevColor(modalReport.aiAnalysis.severity?.level || modalReport.aiAnalysis.severityLevel) }">
                ...
                <div class="analysis-score-card" :style="{ borderTop: `4px solid ${sevColor(modalReport.aiAnalysis.urgency?.level || modalReport.aiAnalysis.urgencyLevel)}` }">
                  <div class="score-header">
                    ...
                    <span class="score-level-badge" :style="{ background: sevColor(modalReport.aiAnalysis.urgency?.level || modalReport.aiAnalysis.urgencyLevel) + '22', color: sevColor(modalReport.aiAnalysis.urgency?.level || modalReport.aiAnalysis.urgencyLevel) }">
              ...
              <p class="paragraph-text">{{ modalReport.aiAnalysis.classification || modalReport.aiAnalysis.disasterClassification }}</p>
              ...
              <div class="findings-container">
                  <div v-if="modalReport.aiAnalysis.keyFindings?.disproportionateImpact" class="finding-block">
                    <div class="f-key" style="color:#00D4FF;">Disproportionate Impact</div>
                    <div class="f-val">{{ modalReport.aiAnalysis.keyFindings.disproportionateImpact }}</div>
                  </div>
                  <div class="finding-block">
                    <div class="f-key" style="color:#00D4FF;">Expert Findings</div>
                  ...
                  <div v-if="modalReport.aiAnalysis.keyFindings?.timeline" class="timeline-box">
                    <div class="timeline-title" style="color:#FFD23F;">Urgency Timeline (Consequences of Delay)</div>
              ...
              <div v-for="rf in modalReport.aiAnalysis.riskFactors" :key="rf.risk || rf" class="risk-matrix-row">
                ...
                <div v-if="rf.mitigation" class="rm-mitigation">
                  ...
                  <div style="font-size:12px;color:#94A3B8;">{{ rf.mitigation }}</div>
                </div>
              </div>
          ...
          <template v-if="modalTab === 'needs'">
            <div v-if="modalReport.needsAssessment" class="fade-up">
              ...
              <div class="score-grid-large" style="margin-top: 8px;">
                <div class="score-box-premium" style="background:#00D4FF12;border:1px solid #00D4FF44;">
                  <div class="sb-lbl">ESTIMATED DURATION</div>
                  <div class="sb-val" style="color:#00D4FF; font-size: 24px;">{{ modalReport.needsAssessment.durationDays }} Days</div>
                  <div class="sb-sub">Coverage Window</div>
                </div>
                <div class="score-box-premium" style="background:#FFD23F12;border:1px solid #FFD23F44;">
                  <div class="sb-lbl">BUDGET ESTIMATE</div>
                  <div class="sb-val" style="color:#FFD23F; font-size: 20px;">{{ modalReport.needsAssessment.totalBudgetEstimate }}</div>
            ...
            <div v-if="modalReport.allocation" class="fade-up">
              ...
              <div class="score-grid-large" style="margin-top: 8px;">
                <div class="score-box-premium" style="background:#00D4FF12;border:1px solid #00D4FF44;">
                  ...
                  <div class="sb-val" style="color:#00D4FF; font-size: 24px;">{{ modalReport.allocation.matchEfficiencyScore }}%</div>
                  <div class="sb-sub">Resource Optimization</div>
                </div>
                <div class="score-box-premium" style="background:#FF6B3512;border:1px solid #FF6B3544;">
                  <div class="sb-lbl">SUPPLY READINESS</div>
                  <div class="sb-val" style="color:#FF6B35; font-size: 24px;">{{ modalReport.allocation.resourceReadinessScore }}%</div>
                ...
                <table class="matching-table-premium">
                  ...
                  <tbody>
                    <tr v-for="(a, i) in modalReport.allocation.allocations" :key="i">
                      <td style="font-weight:700; color:#E2EAF4;">{{ a.barangay }}</td>
                      <td class="mono-cyan">{{ a.foodPacks }} pk</td>
                      <td class="mono-cyan">{{ a.waterLiters }} L</td>
                      <td>
                        <span class="score-level-badge" :style="{ background: sevColor(a.priority) + '22', color: sevColor(a.priority) }">
                          {{ a.priority }}
                        </span>
                      </td>
                      <td style="color: #4A6080; font-size: 11px;">{{ a.source || a.warehouse }}</td>
                    </tr>
                  </tbody>
                </table>
              </div>
              ...
              <div v-for="(g, i) in modalReport.allocation.gapsIdentified" :key="i" class="gap-card-premium">
                <AlertTriangle :size="14" style="color:#FF3B5C; margin-right:10px;" />
                <span style="font-size:12.5px; color:#94A3B8;">{{ g }}</span>
              </div>
...
const statusColor = (s) => ({ pending: '#FFD23F', approved: '#00D4FF', distributed: '#00E5A0', rejected: '#FF3B5C' })[s || 'pending'] || '#4A6080'
...
.alert-bar  { display:flex; align-items:center; gap:10px; background:#FF3B5C15; border:1px solid #FF3B5C44; border-radius:6px; padding:10px 16px; margin-bottom:16px; }
.alert-icon { color:#FF3B5C; font-size:16px; }
.alert-text { font-size:13px; color:#FF3B5C; font-weight:600; flex:1; }
.alert-btn  { background:#FF3B5C; color:#fff; border:none; border-radius:4px; padding:4px 14px; font-size:12px; font-weight:600; cursor:pointer; }
...
.stat-label { font-size:11px; color:#4A6080; font-family:'DM Mono',monospace; letter-spacing:0.06em; text-transform:uppercase; }

.card        { background:#0D1219; border:1px solid #1A2535; border-radius:6px; padding:1.25rem; }
.card-header { display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:16px; }
.card-title  { font-size:14px; font-weight:700; }
.card-sub    { font-size:11px; color:#4A6080; margin-top:2px; }

.table           { width:100%; border-collapse:collapse; }
.table-head-row  { border-bottom:1px solid #1A2535; }
.th              { text-align:left; padding:7px 10px; font-size:10px; color:#4A6080; font-family:'DM Mono',monospace; font-weight:400; letter-spacing:0.06em; white-space:nowrap; }
...
.report-meta { font-size:10px; color:#4A6080; font-family:'DM Mono',monospace; margin-top:1px; }
...
.result-btn-sub   { font-size:10px; color:#4A6080; font-family:'DM Mono',monospace; margin-top:2px; }
.result-btn-link  { font-size:10px; color:#00D4FF; margin-top:2px; }
.result-btn-preview { font-size:9px; color:#4A6080; margin-top:2px; font-style:italic; line-height:1.2; max-width:140px; }

.loading-cell { display:flex; align-items:center; gap:8px; font-size:11px; color:#4A6080; font-family:'DM Mono',monospace; }
.spinner { width:12px; height:12px; border:2px solid #1A2535; border-top-color:#00D4FF; border-radius:50%; animation: spin 1s linear infinite; }
@keyframes spin { to { transform: rotate(360deg); } }

.empty-btn        { background:#060A0F; border:1px dashed #1A2535; border-radius:6px; padding:6px 10px; cursor:pointer; color:#4A6080; font-size:11px; font-family:'DM Mono',monospace; min-width:110px; transition:all .15s; }
.empty-btn:hover  { border-color:#00D4FF; color:#00D4FF; }

.pipeline-dot   { width:10px; height:10px; border-radius:2px; flex-shrink:0; }
.pipeline-count { font-size:10px; color:#4A6080; font-family:'DM Mono',monospace; margin-top:3px; }

.two-col { display:grid; grid-template-columns:1.6fr 1fr; gap:16px; }
.report-row { display:flex; align-items:center; gap:12px; padding:10px 0; border-bottom:1px solid #1A2535; }

.ghost-btn        { display:inline-flex; align-items:center; background:transparent; border:1px solid #1A2535; color:#E2EAF4; padding:5px 12px; border-radius:4px; font-size:12px; cursor:pointer; transition:all .2s; font-family:'Outfit',sans-serif; }
.ghost-btn:hover  { border-color:#00D4FF44; color:#00D4FF; }
...
.modal-backdrop { position:fixed; inset:0; background:rgba(0,0,0,0.75); z-index:999; display:flex; align-items:center; justify-content:center; backdrop-filter:blur(4px); }
.modal          { background:#0D1219; border:1px solid #1A2535; border-radius:8px; width:min(820px,96vw); max-height:88vh; display:flex; flex-direction:column; overflow:hidden; }
.modal-header   { padding:1rem 1.5rem; border-bottom:1px solid #1A2535; display:flex; align-items:center; gap:12px; background:#060A0F; flex-shrink:0; }
.modal-title    { font-weight:800; font-size:15px; }
.modal-sub      { font-size:11px; color:#4A6080; font-family:'DM Mono',monospace; }
.close-btn      { background:none; border:1px solid #1A2535; color:#4A6080; width:28px; height:28px; border-radius:4px; font-size:14px; display:flex; align-items:center; justify-content:center; cursor:pointer; transition:all .2s; }
.close-btn:hover{ border-color:#FF3B5C44; color:#FF3B5C; }
.modal-tabs     { display:flex; border-bottom:1px solid #1A2535; background:#060A0F; flex-shrink:0; }
...
.score-box      { background:#060A0F; border:1px solid #1A2535; border-radius:6px; padding:14px 18px; }
.score-label    { font-size:10px; color:#4A6080; font-family:'DM Mono',monospace; letter-spacing:0.08em; margin-bottom:4px; }
.score-value    { font-size:22px; font-weight:900; line-height:1; margin-bottom:2px; }
.score-sub      { font-size:11px; color:#4A6080; margin-bottom:6px; }
.field-label    { font-size:11px; color:#4A6080; font-family:'DM Mono',monospace; letter-spacing:0.06em; margin-bottom:4px; }
.findings-box   { font-size:13px; line-height:1.75; padding:10px 14px; background:#060A0F; border-left:3px solid #00D4FF; border-radius:4px; white-space:pre-wrap; }
...
.empty-state    { text-align:center; padding:3rem; color:#4A6080; font-size:13px; display:flex; flex-direction:column; align-items:center; }

/* ── EXPERT MODAL STYLES ── */
.score-grid-large { display: grid; grid-template-columns: 1fr 1fr; gap: 16px; margin-bottom: 24px; }
.analysis-score-card { background: #060A0F; border: 1px solid #1A2535; border-radius: 8px; padding: 20px; }
...
.progress-bg { height: 6px; background: #0D1219; border-radius: 3px; margin-top: 8px; overflow: hidden; }
...
.paragraph-text { font-size: 13.5px; line-height: 1.6; color: #E2EAF4; margin-bottom: 16px; }
...
.timeline-box { background: #0D1219; border: 1px solid #1A2535; border-radius: 6px; padding: 16px; margin-top: 8px; }
...
.t-hour { font-size: 9px; font-weight: 800; color: #E2EAF4; background: #1A2535; padding: 4px 8px; border-radius: 3px; align-self: flex-start; }
.t-desc { font-size: 11.5px; line-height: 1.4; color: #4A6080; }

.needs-grid-detailed { display: grid; grid-template-columns: repeat(auto-fill, minmax(260px, 1fr)); gap: 12px; }
.need-detail-card { background: #060A0F; border: 1px solid #1A2535; border-radius: 8px; padding: 16px; }
...
.phase-card { background: #0D1219; border: 1px solid #1A2535; border-radius: 6px; overflow: hidden; }
...
.risk-matrix-row { display: flex; gap: 16px; padding: 16px; background: #060A0F; border: 1px solid #1A2535; border-radius: 8px; margin-bottom: 8px; }
.rm-main { flex: 1; }
...
.rm-mitigation { flex: 1.2; border-left: 1px solid #1A2535; padding-left: 20px; }
.rm-m-lbl { font-size: 10px; font-weight: 700; color: #00E5A0; margin-bottom: 4px; text-transform: uppercase; }

.score-box-premium { border-radius: 8px; padding: 20px; text-align: center; }
.sb-lbl { font-size: 10px; color: #4A6080; font-family: 'DM Mono', monospace; margin-bottom: 6px; }
.sb-val { font-weight: 800; line-height: 1.1; margin-bottom: 4px; }
.sb-sub { font-size: 10px; color: #4A6080; font-family: 'DM Mono', monospace; }

.vuln-box-premium { background: #FF6B3512; border: 1px solid #FF6B3544; border-radius: 6px; padding: 16px; color: #FF6B35; font-size: 14px; line-height: 1.6; font-weight: 500; display: flex; align-items: center; }

.reasoning-grid-premium { display: flex; flex-direction: column; gap: 10px; }
.reason-pill { background: #060A0F; border: 1px solid #1A2535; border-radius: 8px; padding: 12px 16px; }
.rp-key { font-size: 11px; text-transform: uppercase; color: #00D4FF; font-weight: 700; margin-bottom: 4px; }
.rp-val { font-size: 13px; line-height: 1.5; color: #94A3B8; }

.tag-premium { display: inline-block; padding: 3px 10px; background: #00E5A010; border: 1px solid #00E5A033; color: #00E5A0; font-size: 11px; font-family: 'DM Mono', monospace; border-radius: 4px; margin-right: 8px; margin-bottom: 8px; }
.pill-legacy { padding: 4px 12px; background: #1A2535; color: #E2EAF4; border-radius: 20px; font-size: 12px; margin-right: 8px; margin-bottom: 8px; border: 1px solid #2D3B4E; }

/* ── Matching Specific Premium Styles ── */
.matching-table-premium { width: 100%; border-collapse: collapse; margin-top: 8px; }
.matching-table-premium th { text-align: left; padding: 12px 8px; font-size: 10px; color: #4A6080; font-family: 'DM Mono', monospace; text-transform: uppercase; border-bottom: 1px solid #1A2535; }
.matching-table-premium td { padding: 12px 8px; border-bottom: 1px solid #1A253520; font-size: 13px; }
.mono-cyan { font-family: 'DM Mono', monospace; color: #00D4FF; font-weight: 600; }

.step-card-premium { display: flex; align-items: flex-start; gap: 12px; background: #060A0F; border: 1px solid #1A2535; border-radius: 6px; padding: 12px 16px; margin-bottom: 8px; }
.step-num-premium { width: 20px; height: 20px; background: #00D4FF18; border: 1px solid #00D4FF33; color: #00D4FF; font-size: 10px; font-weight: 800; border-radius: 50%; display: flex; align-items: center; justify-content: center; flex-shrink: 0; margin-top: 2px; }
.step-text-premium { font-size: 12.5px; color: #E2EAF4; line-height: 1.5; }

.gap-card-premium { display: flex; align-items: center; background: #FF3B5C0a; border: 1px solid #FF3B5C22; border-radius: 6px; padding: 10px 16px; margin-bottom: 6px; }
